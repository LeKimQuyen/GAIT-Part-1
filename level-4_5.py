...
pygame.display.set_caption("GridWorld - Part 1 Q-learning, Level 4-5")
...
# Level 4 layout
LEVEL4 = load_layout(4)
LEVEL4 = [row.ljust(WIDTH_TILES)[:WIDTH_TILES] for row in LEVEL4]
...
class GridWorld:
    def __init__(self, layout: List[str]):
        ...
        self.monster_init, self.monsters = [], []
        self.start = (0, 0)
        # Animation
        self.apple_imgs, self.fire_imgs, self.rock_img, self.key_img, self.chest_imgs, self.monster_img = load_images()
        self.apple_state = self.chest_state = 0
        ...
        for y, row in enumerate(layout):
            for x, ch in enumerate(row):
                ...
                elif ch == "M": self.monster_init.append(p)
    ...
    def reset(self) -> Tuple[int, int, int, int, int]:
        ...
        for i in range(len(self.apples)):
            self.apple_mask |= (1 << i)
        # Build an key mask with one bit per key, all set to 1 at start
        self.key_mask = 0
        for i in range(len(self.keys)):
            self.key_mask |= (1 << i)
        self.collected_keys = 0
        # Build an chest mask with one bit per chest, all set to 1 at start
        self.chest_mask = 0
        for i in range(len(self.chests)):
            self.chest_mask |= (1 << i)
        self.chest_state = 0
        self.step_count = 0
        self.monsters = list(self.monster_init)
        return self.encode_state()

    def encode_state(self) -> Tuple[int, int, int, int, int]:
        # Task 3: encode for key and chest
        return (
            self.agent[0],
            self.agent[1],
            self.apple_mask,
    ...
    def step(self, action: int) -> StepResult:
        ...
        # NEW: Key collection (after apple collection)
        ...
        # NEW: Chest opening (requires key!)
        if self.agent in self.chest_index and self.collected_keys > 0:
            idx = self.chest_index[self.agent]
            if (self.chest_mask >> idx) & 1:  # Check if closed
                self.chest_mask &= ~(1 << idx)  # Open it
                self.collected_keys -= 1  # Use the key
                reward += 2.0  # +2 reward for chest!
                self.chest_state = 1 # animation
        ...
        # 4) Episode end condition when all apples and chests gone
    ...
    def update_monsters(self):
        ...
        for m in self.monsters:
            if random.random() < 0.4:   # 40% change to move
                # Find legal positions
                positions = [self.try_move(m, action) for action in ALL_ACTIONS]
                m = random.choice(positions)  # Pick randomly a legal position
...
# Task 2: choose next state, not state of max value
...
def draw_grid(env: GridWorld, episode, step, epsilon, total_reward):
    ...
    for p in env.rocks:
        cx, cy = p[0]*TILE_SIZE + TILE_SIZE//2, p[1]*TILE_SIZE + TILE_SIZE//2
        rock_rect = env.rock_img.get_rect(center=(cx, cy))
        screen.blit(env.rock_img, rock_rect)
    ...
    # NEW: Draw chests (only if not opened)
    for p, idx in env.chest_index.items():
        cx, cy = p[0]*TILE_SIZE + TILE_SIZE//2, p[1]*TILE_SIZE + TILE_SIZE//2
        chest_rect = env.chest_imgs[env.chest_state].get_rect(center=(cx, cy))
        screen.blit(env.chest_imgs[env.chest_state], chest_rect)

    # Monsters
    for p in env.monsters:
        cx, cy = p[0]*TILE_SIZE + TILE_SIZE//2, p[1]*TILE_SIZE + TILE_SIZE//2
        monster_rect = env.monster_img.get_rect(center=(cx, cy))
        screen.blit(env.monster_img, monster_rect)
    ...
    hud = [
        ...
        f"Return {total_reward:.2f}  Part 1 Q-learning Level 2-3",
...
def load_images():
    ...
    # Apples
    apple0 = pygame.transform.scale(pygame.image.load(path + 'apple-0.png').convert_alpha(), (TILE_SIZE // 1.5, TILE_SIZE // 1.5))
    apple1 = pygame.transform.scale(pygame.image.load(path + 'apple-1.png').convert_alpha(), (TILE_SIZE // 1.5, TILE_SIZE // 1.5))
    ...
    # Rock
    rock = pygame.transform.scale(pygame.image.load(path + 'rock.png').convert_alpha(), (TILE_SIZE * 1.5, TILE_SIZE * 1.5))
    ...
    # Monster
    monster = pygame.transform.scale(pygame.image.load(path + 'monster.png').convert_alpha(), (TILE_SIZE, TILE_SIZE))
...
def run_training():
    ...
    for ep in range(EPISODES):
        ...
        # Task 2: choose action before loop
